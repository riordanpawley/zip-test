...
export const useShopFeedWithSuspense = () => {
	...
	const { data, networkStatus } = useSuspenseQuery(searchShopFeedDocument, {
		variables: { query: debouncedSearch },
		errorPolicy: "none",
	});
	console.log({ search, networkStatus, data });
...
export const useShopFeedWithoutSuspense = () => {
	const { search } = useContext(SearchContext);
	const [debouncedSearch] = useDebounceValue(search, 300);
	const { data, networkStatus } = useQuery(searchShopFeedDocument, {
		variables: { query: debouncedSearch },
		errorPolicy: "none",
	});
